const curriculo = {
  nome: "João Gabriel Vieira Silva",
  foto: "foto.jpg",
  info: `
    Idade: 20 anos<br>
    Telefone: (83) 99408-5691<br>
    Email: oosoapy@gmail.com<br>
    Endereço: Rua Francisca Fernandes Claudino, 440 - Cajazeiras, Paraíba, Brasil
  `,

  formacao: [
    "Pós Graduação em Docência do Ensino Superior (em andamento)",
    "Curso Superior em Análise e Desenvolvimento de Sistemas (em andamento) - IFPB (6º período)",
    "Ensino Médio Completo (2022)"
  ],

  experiencia: [
    {
      empresa: "Mondragon Unibertsitatea",
      cargo: "Pesquisador na área de Inteligência Artificial",
      periodo: "09/2025 – 01/2026",
      descricao: "Pesquisa e desenvolvimento de projetos relacionados à inteligência artificial, incluindo análise de dados, desenvolvimento de modelos, leitura e escrita de artigos científicos, participação em conferências e colaboração com equipes multidisciplinares."
    },
    {
      empresa: "Fundação de Apoio à Pesquisa do Estado da Paraíba (FAPESQ)",
      cargo: "Bolsista de Iniciação Científica",
      periodo: "09/2025 – 01/2026",
      descricao: "Representar a Paraíba na Espanha, cursando engenharia informática, contribuindo para o avanço do conhecimento científico e promovendo a colaboração internacional."
    },
    {
      empresa: "Loopis Jr",
      cargo: "Diretor de Recursos Humanos",
      periodo: "10/2023 – 09/2025",
      descricao: "Elaboração de documentos, levantamento de requisitos, análise de mercado e prospecção de clientes."
    },
    {
      empresa: "Infolight Tecnologia da Informação LTDA",
      cargo: "Cargo: Analista de Sistemas estagiário",
      periodo: "09/2024 – 03/2025",
      descricao: "Criação de sistemas, testes, documentação técnica, suporte ao cliente, manutenção, implantação de software e manutenção de XML."
    },
    {
      empresa: "Premium Brindes",
      cargo: "Auxiliar de Produção",
      periodo: "12/2022 – 02/2023",
      descricao: "Atendimento ao cliente, confecção de artes, vetorização de artes, operar máquinas de serigrafia e produção de brindes."
    }
  ],

  extracurricular: [
    {
      titulo: "Monitor voluntário – Algoritmos e Lógica de Programação",
      periodo: "Carga horária: 6 horas semanais. 11/2024 – 05/2025",
      descricao: "Auxiliar alunos a obter o conhecimento necessário para a cadeira de Algoritmos e Lógica de programação, envolvendo aulas, resoluções de questões e até mesmo prototipação de algoritmos."
    },
    {
      titulo: "Professor do Minicurso: Desvendando Lua",
      periodo: "Carga horária para os alunos: 2 horas. 12/2024",
      descricao: "Apresentando a linguagem Lua desde os seus conceitos mais básicos até o consumo de APIs."
    },
    {
      titulo: "Concluinte do ciclo Pré Intermediate 3 do curso More English",
      periodo: "Carga horária: 360 horas. 2018 - 2019",
      descricao: "Aprendendo inglês de forma lúdica, com foco em conversação e compreensão oral."
    }
  ],

  marcos: [
    "Medalhista de bronze na 25ª Olimpíada Brasileira de Astronomia e Astronáutica",
    "Equipe 2º lugar no Ideathon do Startup Day 2026, financiado pelo Sebrae Startups",
    "Equipe 2º lugar no Hackathon do IV SertãoComp, financiado pelo IFPB e a prefeitura de Cajazeiras",
    "Participação das edições IV e V SertãoComp, evento de tecnologia e inovação do IFPB",
    "Minicurso Hyperledger Fabric: Hiperledger Fabric: Primeiros passos com Blockchain",
    "Minicurso Animações Dinâmicas: Do 2D ao 3D com GSAP e Three.js",
    "Participação da palestra: Suporte Básico De Vida (SBV), os 4 passos que salvam vidas",
    "Monitor voluntário do projeto de extensão focado em esportes: De Rede Em Rede, do IFPB",
    "Participação do clube de leitura do Colégio Nossa Senhora de Lourdes",
    "Certificados de \"Redações + Tops\" na primeira, segunda e terceira edição do concurso de simulados do Colégio Nossa Senhora de Lourdes",
    "1º lugar no Campeonato Interno de Xadrez (Categoria A) nas edições de 2017, 2018 e 2019",
    "Criação de projetos extracurriculares, disponível em: https://github.com/Osoapy"
  ],

  habilidades: [
    "Português (Nativo)",
    "Inglês (Avançado)",
    "Espanhol (Intermediário)",
    "Libras (Básico)",
    "Entusiasta em TI e IA",
    "Pensamento estratégico",
    "Fácil adaptação",
  ]
};
